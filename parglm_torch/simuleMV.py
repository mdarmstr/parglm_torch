...
def Floc(x, xdata):
    ...
    y = np.zeros(xdata.shape[0], dtype=np.float64)
...
def ADICOV(covariance_matrix, data_matrix):
    ...
    try:
        ...
        eps = 1e-6
        covariance_matrix += eps * np.eye(covariance_matrix.shape[0])
        L = np.linalg.cholesky(covariance_matrix)
    X = data_matrix @ L.T
    return X
...
def simuleMV(obs, vars, LevelCorr=5, Covar=None):
    ...
    if uselevel:
        if LevelCorr > 0:
            ...
            data_matrix = np.random.randn(int(obs2), vars)
            X = ADICOV(np.eye(vars), data_matrix)
            # Preprocess X
            Xs = X - X.mean(axis=0, keepdims=True)
            # Compute covariance
            COV = (Xs.T @ Xs) / (X.shape[0] -1)
            # Adjust covariance matrix
            corM = COV + 0.01 * np.eye(vars)
        else:
            if obs < vars:
                print('Warning: correlation level too low. Resulting matrix may show a higher correlation due to structural constraints.')

    # Generate final data
    data_matrix = np.random.randn(obs, vars)
